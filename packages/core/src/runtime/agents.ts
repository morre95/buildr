...
export class MainAgentSession {
  ...
  async run(): Promise<MainAgentSessionReport> {
    ...
    const subAgents = await runBoundedParallel(
      this.options.tasks,
      maxParallel,
      async (task) => {
        const result = await this.runSubAgent(task, compactedContext);
  ...
  private async runSubAgent(task: SubAgentTask, compactedContext: CompactedAgentContext): Promise<SubAgentResult> {
    try {
      ...
      const rewriteOptions = {
        goal: [
          ...
          "",
          "Compacted main-agent context:",
          compactedContext.text
        ].join("\n"),
      ...
      const contextSummary = task.contextSummary ?? this.options.contextSummary;
